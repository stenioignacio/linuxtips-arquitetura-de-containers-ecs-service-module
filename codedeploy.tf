...
resource "aws_codedeploy_deployment_group" "main" {
  ...
  app_name              = aws_codedeploy_app.application[count.index].name
  deployment_group_name = aws_codedeploy_app.application[count.index].name
  ...
  load_balancer_info {
    target_group_pair_info {
      prod_traffic_route {
        listener_arns = [data.aws_ssm_parameter.service-listener.value]
      }
