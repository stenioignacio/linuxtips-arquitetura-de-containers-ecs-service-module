resource "aws_cloudwatch_metric_alarm" "rollback_alarm" {
  ...
  alarm_name = format("%s-error-rate", aws_codedeploy_app.application[count.index].name)
  ...
  //Error Rate - Qtd Erros / Qtd Requests * 100
  //(errBlue + errGreen) / (qtdBlue + qtdGreen) * 100
  ...
  metric_query {
    ...
    metric {
      ...
      dimensions = {
        LoadBalancer = data.aws_ssm_parameter.lb_arn.value
        TargetGroup  = aws_alb_target_group.blue[0].arn_suffix
      }
    }
  }
  ...
  metric_query {
    ...
    metric {
      ...
      dimensions = {
        LoadBalancer = data.aws_ssm_parameter.lb_arn.value
        TargetGroup  = aws_alb_target_group.green[0].arn_suffix
      }
    }
  }
