resource "local_file" "appspec" {
  ...
  filename = "${path.module}/${aws_codedeploy_app.application[count.index].name}.yaml"
  content = templatefile("${path.module}/files/codedeploy/appspec.yaml.tpl", {
    APPLICATION_NAME    = aws_codedeploy_app.application[count.index].name
...
resource "null_resource" "deploy_codedeploy" {
  ...
  provisioner "local-exec" {
    command = "aws deploy create-deployment --cli-input-yaml file://${path.module}/${aws_codedeploy_app.application[count.index].name}.yaml --profile lab"
    environment = {
  ...
  depends_on = [aws_ecs_service.main,
    ...
    aws_codedeploy_app.application,
